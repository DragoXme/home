...
document.addEventListener("DOMContentLoaded", function () {
  (function () {
    ...
    textinside.addEventListener("keypress", function (e) {
      if (e.key === "Enter") {
        let inputValue = textinside.value.trim();
        // Check if the input looks like a URL
        if (/^(https?|ftp):\/\/\S+|^(\S+\.)+\S+$/.test(inputValue)) {
          // Prepend "http://" to the input value
          ...
          // Submit the search query to the search engine
          searchForm.submit();
        }
      }
    });
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  button2.addEventListener("click", function() {
    openLinkInSameTab("https://gemini.google.com/");
  });
